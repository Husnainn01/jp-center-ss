...
const FILTER_TTL = 2 * 60 * 1000; // 2 min
...
filterOptionsRouter.get("/", async (req, res) => {
  try {
    ...
    if (maker) {
      // Maker-specific options — cached per maker (and optionally per model)
      const cacheKey = model ? `filter:maker:${maker}:model:${model}` : `filter:maker:${maker}`;
    ...
    // Global filter options — cached
    const cacheKey = includeAll ? "filter:global:all" : "filter:global";
    const data = await cached(cacheKey, async () => {
